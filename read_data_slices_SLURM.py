...
start_t = 0
x_plane = 0
for z_plane in range(9,10):  #Jinyuan used 10, I had 50 before
    gaussian = '/gpfs/fs0/scratch/j/jphickey/g2malik/working_code/GMM_Database/Results0/gaussian1 %d %d %d.dat'%(start_t, x_plane+1, z_plane)
    g = open(gaussian, "w") #Includes properties of gaussain curves

    spatial = '/gpfs/fs0/scratch/j/jphickey/g2malik/working_code/GMM_Database/Results0/spatial1 %d %d %d.dat'%(start_t, x_plane+1, z_plane)
    s = open(spatial, "w") #Includes height and spanwise velocity of spatial UMZ

    quadrant = '/gpfs/fs0/scratch/j/jphickey/g2malik/working_code/GMM_Database/Results0/quadrant1 %d %d %d.dat'%(start_t, x_plane+1, z_plane)
    ...
    for snap in range (start_t,1):
        ...
        y_log_min = yod_event_profile[i_log_min]
        ...
        plt.title("010%02d mean U X#%d at Zlabel = %d"%(snap,x_plane+1,z_plane))
        ...
        plt.ylabel("y")
        #plt.xlim([-0.02, 0.02])
        plt.title("010%02d mean V X#%d at Zlabel = %d"%(snap,x_plane+1,z_plane))
        ...
        plt.ylabel("y")
        #plt.xlim([-0.02, 0.02])
        plt.title("010%02d U'V' X#%d at Zlabel = %d"%(snap,x_plane+1,z_plane))
        #plt.savefig('/gpfs/fs0/scratch/j/jphickey/g2malik/working_code/Q2_Detection/Results/mean/010%02d mean V x#%d Zlabel %d.png'%(ii,x_plane+1,z_plane), facecolor='w')
        plt.show()
        plt.close()
